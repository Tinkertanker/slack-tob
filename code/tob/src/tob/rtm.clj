...
(defn- ping-pong-loop [{:keys [state] :as client}]
      ...
      (d/let-flow [pinged? (s/take! pings)
        ...
        (if (and pinged? ponged?)
          ...
          (do
            ...
            (log/info "Connection closed? " (s/closed? @(:conn state)))
            (ws-recon client)))))))
...
(defmethod event-handler "user_typing" [event {:keys [state] :as client}]
  (send-msg @(:conn state) {:id @(:count state)
                            :type "message"
